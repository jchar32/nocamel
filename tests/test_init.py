...
class TestNoCamelCLI:
    ...
    def test_kebab_case_option(self, cli_runner):
        """Test the --kebab option."""
        # Use exactly the same pattern as the working tests
        with patch("nocamel.converters.to_kebab", return_value="test-result") as mock_convert:
            with patch.object(pyperclip, "copy") as mock_copy:
                result = cli_runner.invoke(main, ["--kebab", "testInput"])
    ...
    def test_multiple_options_error(self, cli_runner):
        ...
        assert result.exit_code != 0
        # Keep the original assertion to match the actual error message
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("--kebab", "to_kebab", "testInput", "test-result"),
        ],
    )
    def test_conversion_options_parametrized(self, option, converter_func, input_text, expected_output, cli_runner):
        ...
        # Return to the original pattern that works for the other tests
